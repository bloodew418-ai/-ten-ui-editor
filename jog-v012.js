(function(){
'use strict';
if(window.__TEN_JOG_012__) return; window.__TEN_JOG_012__=true;
function $(id){return document.getElementById(id)}
var oldFab=$('quickFab'); if(!oldFab) return;
var fab=oldFab.cloneNode(true); oldFab.parentNode.replaceChild(fab,oldFab);
var brand=document.querySelector('.brand span'); if(brand) brand.textContent='v0.1.2';
...
var step=360/ITEMS.length,target=225,rot=0,drag=false,moved=false,startA=0,startR=0;
function norm(v){v%=360;return v<0?v+360:v} function delta(a,b){var d=norm(a-b);return d>180?d-360:d}
...
function draw(){
 var box=$('tenJogItems'); if(!box.children.length){ITEMS.forEach(function(it,i){var b=document.createElement('button');b.className='ten-jog-item';b.dataset.j=i;b.innerHTML='<b>'+it.i+'</b><span>'+it.l+'</span>';b.addEventListener('click',function(ev){ev.stopPropagation();if(!moved) choose(it.m)});box.appendChild(b)})}
...
function closeJog(){jog.classList.remove('open');jog.setAttribute('aria-hidden','true');fab.classList.remove('jog-open')}
...
fab.addEventListener('click',openJog);$('tenJogClose').addEventListener('click',function(ev){ev.stopPropagation();closeJog()});if(close)close.addEventListener('click',closeSheet);
jog.addEventListener('pointerdown',function(ev){if(ev.target.closest('#tenJogClose')||ev.target.closest('.ten-jog-item'))return;drag=true;moved=false;startA=angle(ev);startR=rot;jog.setPointerCapture(ev.pointerId)});
jog.addEventListener('pointermove',function(ev){if(!drag)return;var d=delta(angle(ev),startA);if(Math.abs(d)>3)moved=true;rot=startR+d;draw()});
jog.addEventListener('pointerup',function(ev){if(!drag)return;drag=false;try{jog.releasePointerCapture(ev.pointerId)}catch(_){}snap();setTimeout(function(){moved=false},70)});jog.addEventListener('pointercancel',function(){drag=false;snap()});
})();
